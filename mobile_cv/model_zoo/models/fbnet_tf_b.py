...
class FBNetBBackbone(tf.keras.layers.Layer):
    def __init__(self, **kwargs):
        super(FBNetBBackbone, self).__init__(**kwargs)
        ...
        self.stages['xif1_0'] = IRF(16, 16, 16, 3, 1, res=True)
        
        # --- Stage 2 ---
        # xif2_0: C_in=16, C_exp=48, C_out=24, k=3, s=2
        self.stages['xif2_0'] = IRF(16, 96, 24, 3, 2)
        # xif2_1: C_in=24, C_exp=24, C_out=24, k=5, s=1 (No PW, Res)
        self.stages['xif2_1'] = IRF(24, 24, 24, 5, 1, dw_padding=2, res=True)
        # xif2_2, xif2_3: Identity
        self.stages['xif2_2'] = IRF(24, 24, 24, 3, 1, res=True)
        self.stages['xif2_3'] = IRF(24, 24, 24, 3, 1, res=True)
        ...
        self.stages['xif3_1'] = IRF(32, 96, 32, 5, 1, dw_padding=2, res=True)
        # xif3_2: C_in=32, C_exp=32, C_out=32, k=5, s=1 (No PW, Res)
        self.stages['xif3_2'] = IRF(32, 192, 32, 3, 1, res=True)
        # xif3_3: C_in=32, C_exp=96, C_out=32, k=3, s=1 (Res)
        self.stages['xif3_3'] = IRF(32, 192, 32, 5, 1, dw_padding=2, res=True)
        ...
        self.stages['xif4_1'] = IRF(64, 384, 64, 5, 1, dw_padding=2, res=True)
        # xif4_2: C_in=64, C_exp=64, C_out=64, k=5, s=1 (Shuffle, No PW, Res, pwl_g=2)
        self.stages['xif4_2'] = tf.identity
        # xif4_3: C_in=64, C_exp=384, C_out=64, k=5, s=1 (Res)
        self.stages['xif4_3'] = IRF(64, 192, 64, 5, 1, dw_padding=2, res=True)
        # xif4_4: C_in=64, C_exp=384, C_out=112, k=3, s=1
        self.stages['xif4_4'] = IRF(64, 384, 112, 5, 1, dw_padding=2)
        # xif4_5: C_in=112, C_exp=112, C_out=112, k=5, s=1 (Shuffle, No PW, Res, pwl_g=2)
        self.stages['xif4_5'] = IRF(112, 112, 112, 3, 1, res=True)
        # xif4_6: C_in=112, C_exp=336, C_out=112, k=5, s=1 (Res)
        self.stages['xif4_6'] = IRF(112, 112, 112, 5, 1, dw_padding=2, res=True)
        # xif4_7: C_in=112, C_exp=112, C_out=112, k=3, s=1 (Shuffle, No PW, Res, pwl_g=2)
        self.stages['xif4_7'] = IRF(112, 336, 112, 5, 1, dw_padding=2, res=True)
        ...
        self.stages['xif5_1'] = IRF(184, 184, 184, 5, 1, dw_padding=2, res=True)
        # xif5_2: C_in=184, C_exp=552, C_out=184, k=5, s=1 (Res)
        self.stages['xif5_2'] = IRF(184, 1104, 184, 5, 1, dw_padding=2, res=True)
        ...
        self.stages['xif5_4'] = IRF(184, 1104, 352, 3, 1)

        # xif6_0: Final Conv
        self.stages['xif6_0'] = ConvBNRelu(1984, 1, strides=1, use_bias=True) # (352, 1504, k=1, s=1)
...
# --- E. FBNet 完整模型 ---
class FBNetBKeras(tf.keras.Model):
    def __init__(self, num_classes=1000, **kwargs):
        super(FBNetBKeras, self).__init__(**kwargs)
        self.backbone = FBNetBBackbone()
